...
public class Lunar {
     ...
     */
    private static int[] lunarToSolar(int year, int month, int monthDay, boolean isLeapMonth) {
        int dayOffset;
        int leapMonth;
        int i;
        ...
        for (i = 1; i < 13; i++) {
            ...
            if (iPos >= dayOffset) {
                ...
                if (dayOffset > iPos) {
                    ...
                    // TODO: 2022/11/25 这里移除了一部分，不知道会不会破环原有逻辑
    ...
    /**
     * 这是该类对外的接口，适配该App的API，将农历几月几号转化为公历几月几号
     * @param lunarDate 农历几月几号的字符串表示
     * @return 公历几月几号的字符串表示
     */
    public static String lunar2solar(String lunarDate) {
        String[] lunarDateArr = lunarDate.split("-");
        int year = Integer.parseInt(lunarDateArr[0]);
        int month = Integer.parseInt(lunarDateArr[1]);
...
//    }
     ...
     * (如果当年有闰月，被闰的阴历月份所对应的阳历日期计算出来不对，后期自己根据本方法leap3 返回值判断如果为1，给月份+1，
     * 如果为0就是正确的，不用在操作，阴历闰月对应的阳历日期计算出来的阴历闰月日期正确，leap3为0)
